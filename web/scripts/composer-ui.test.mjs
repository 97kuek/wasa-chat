...
// ⚠️ **グリッドの列は必ず書く。** 書かないと暗黙の列が `auto` になり、
// いちばん縮まない子の最小幅まで広がる。実測（2026-09-13、実アプリを
// Chromeで測定）では、ヘッダーが縮めないせいで列が412.6pxになり、
// ヘッダー・会話・入力欄が揃って画面より広くなっていた:
//
//	320px → 378px（+58）／393px → 413px（+20）／768px → 789px（+21）
//
// body は overflow:hidden なので横スクロールは出ず、**右端が黙って切れる**。
// お知らせと連携メニューが切れて見えたのはこれが原因だった。
test("チャットの列を画面幅に固定する（右端が黙って切れない）", () => {
  assert.match(styles, /\.chat-panel\s*\{[^}]*grid-template-columns:\s*minmax\(0, 1fr\);/s);
});
