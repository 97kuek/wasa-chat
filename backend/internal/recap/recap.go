...
// 出力の形。**Discordは表もMermaidの図もレンダリングしない**ので、どちらの
// 仕事でも同じ制約がかかる（internal/discord のパッケージ説明を参照）。
const outputRules = `
# 出力の規則

- 日本語で書く。Discordのメッセージとして読まれる
- 全体を1000文字以内に収める。前置きは書かない
- 表は使わない。図（mermaid）は使わない。Discordでは崩れる
- 発言者の名前は会話ログの表記のまま使う`
...
` + outputRules + `
- 「決まったこと」「未決のこと」「誰が何をやることになったか」の順に箇条書きで書く
- 該当が無い項目は、見出しごと省く
